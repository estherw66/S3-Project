package com.fontys.s3itproject.configuration.exceptionhandler;
...
public class RestCustomExceptionHandler extends ResponseEntityExceptionHandler {
    private static final String INTERNAL_SERVER_ERROR_MESSAGE = "Internal service error.";
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(final MethodArgumentNotValidException error, final HttpHeaders headers, final HttpStatus status, final WebRequest request) {
        log.error("MethodArgumentNotValidException with status {} occurred {}", kv(STATUS_LOG_FIELD, HttpStatus.BAD_REQUEST), kv(ERROR_LOG_FIELD, error));
    ...
    public ResponseEntity<Object> handleConstraintViolationException(final ConstraintViolationException error) {
        log.error("ConstraintViolationException with status {} occurred {}", kv(STATUS_LOG_FIELD, HttpStatus.BAD_REQUEST), kv(ERROR_LOG_FIELD, error));
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(convertToResponseBody(error));
    }

    @ExceptionHandler(value = {ResponseStatusException.class})
    public ResponseEntity<Object> handleResponseStatusException(final ResponseStatusException error, final WebRequest request) {
        log.error("ResponseStatusException with status {} occurred {}", kv(STATUS_LOG_FIELD, error.getStatus()), kv(ERROR_LOG_FIELD, error));
        return ResponseEntity.status(error.getStatus()).body(error.getReason());
    }

    @ExceptionHandler(value = {RuntimeException.class})
    public ResponseEntity<Object> handleUnknownRuntimeError(final RuntimeException error, final WebRequest request) {
        log.error("Internal server error occurred {}", kv(ERROR_LOG_FIELD, error));
